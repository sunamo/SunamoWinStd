...
public class ReparsePoint
{
    ...
    [return: MarshalAs(UnmanagedType.Bool)]
    static extern bool OpenProcessToken(nint ProcessHandle,
    ...
    [DllImport("advapi32.dll", SetLastError = true)]
    [return: MarshalAs(UnmanagedType.Bool)]
    static extern bool AdjustTokenPrivileges(nint TokenHandle,
    ...
    public ReparsePoint(string path)
    {
        ...
        if (success)
        {
            ...
            if (success)
            {
                ...
                success = AdjustTokenPrivileges(token, false, ref tokenPrivileges, Marshal.SizeOf(tokenPrivileges), nint.Zero, nint.Zero);
                lastError = Marshal.GetLastWin32Error();
            }
            CloseHandle(token);
        }
        ...
        if (success)
        {
            ...
            nint handle = CreateFile(path, FileAccess.Read, FileShare.None, 0, FileMode.Open, FILE_FLAG_OPEN_REPARSE_POINT | FILE_FLAG_BACKUP_SEMANTICS, nint.Zero);
            lastError = Marshal.GetLastWin32Error();
            if (handle.ToInt32() >= 0)
            {
                ...
                uint bytesReturned;
                success = DeviceIoControl(handle, controlCode, nint.Zero, 0, out buffer, MAXIMUM_REPARSE_DATA_BUFFER_SIZE, out bytesReturned, nint.Zero);
                lastError = Marshal.GetLastWin32Error();
                if (success)
                {
                    string subsString = "";
